...
public class EmailListenerManager {
    ...
    public void startListening(String emailService, ScriptableFunction callback) {
        try {
			//callback.invoke(null, new Object[] { "start listening" + emailService });
			if (!emailService.equalsIgnoreCase("")) {			
			    ServiceBook sb = ServiceBook.getSB();
				ServiceRecord[] records = sb.findRecordsByCid("CMIME");
				//callback.invoke(null, new Object[] { "service books count" + records.length });
				if (records != null){
					for (int i = 0; i < records.length; ++i){
						ServiceRecord record = records[i];  
						//callback.invoke(null, new Object[] { "service record name" + record.getName() });
						  ...
						  if (record.isValid())
							{ 
							  ...
							  { 
								  ...
								  //callback.invoke(null, new Object[] { "names match" });
									...
									if (sc != null)
									{
										...
										if (session != null)
										{
											//callback.invoke(null, new Object[] { "session not null" });
										  ...
										  if (_listenerHash.get(emailService)==null) {
												...
												//callback.invoke(null, new Object[] { "added listener to hash" });
												...
												//callback.invoke(null, new Object[] { "added to vector" });
